...
pred_label = knn.predict(test_target)

print("模型精度：{:.2f}".format(np.mean(pred_label == test_label)))
print("模型精度：{:.2f}".format(knn.score(test_target, test_label)))

```

我们知道数据集中有40000条数据，在多次测试后知道在n_neighbors=101附近时有最高的精度89%至91%
...
​        首先实现两个辅助函数：一个用于零填充，另一个用于计算卷积同时进行偏置。 边界填充将 会在图像边界周围添加值为0的像素点.使用0填充边界。
...
卷积公式：
![360截图20230304144429275](C:\Users\Lenovo\Desktop\360截图20230304144429275.jpg)
...
最大值池化层：在输入矩阵中滑动一个大小为fxf的窗口，选取窗口里的值中的最大值，然后作为输出的一部分。-- （这里进行使用） --
...
卷积运算的处理流，而池化层是缩小高、长空间上的运算。取出2x2区域中的最大值元素。
...
if __name__ == '__main__':
    (train_target, train_label), (test_target, test_label) = load_mnist(flatten=False)
    # 处理花费时间较长的情况下减少数据
    ...
    for i in range(iter_nums):
        ...
        x_batch = train_target[batch_mask]
        ...
        if i % iter_per_epoch == 0:
            # 测试在所有训练数据和测试数据上的准确率
            train_acc = net.accuracy(train_target, train_label)
            test_acc = net.accuracy(test_target, test_label)
